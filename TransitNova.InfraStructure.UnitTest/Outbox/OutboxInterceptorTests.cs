...
{
    [Fact]
    public async Task ConvertDomainEventsToOutboxMessages_DomainEvent_Should_CreateCompleteOutboxMessage()
    {
    ...
    [Fact]
    public async Task ConvertDomainEventsToOutboxMessages_FirstSave_Should_ClearAggregateEvents()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync(
            new ConvertDomainEventsToOutboxMessages());
    ...
    [Fact]
    public async Task ConvertDomainEventsToOutboxMessages_FirstSave_Should_LeaveMessageUnpersistedUntilNextSave()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync(
            new ConvertDomainEventsToOutboxMessages());
        ...
        fixture.Context.ChangeTracker.Entries<OutboxMessage>()
            .Should().ContainSingle(x => x.State == EntityState.Added);
        (await fixture.Context.OutboxMessages.AsNoTracking().CountAsync()).Should().Be(0,
            "messages added from SavedChangesAsync are not part of the completed database save");
    }

    [Fact]
    public async Task ConvertDomainEventsToOutboxMessages_SecondSave_Should_PersistOneMessageWithoutDuplication()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync(
            new ConvertDomainEventsToOutboxMessages());
        fixture.Context.Bundles.Add(CreateBundleWithSubscription(out _));
    ...
    [Fact]
    public async Task ConvertDomainEventsToOutboxMessages_NoDomainEvents_Should_NotCreateMessage()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync(
            new ConvertDomainEventsToOutboxMessages());
    ...
    [Fact]
    public async Task ConvertDomainEventsToOutboxMessages_TwoDomainEvents_Should_CreateMessagesInOneBatch()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync(
            new ConvertDomainEventsToOutboxMessages());
        ...
        fixture.Context.ChangeTracker.Entries<OutboxMessage>()
            .Should().HaveCount(2)
            .And.OnlyContain(x => x.State == EntityState.Added);
    }
