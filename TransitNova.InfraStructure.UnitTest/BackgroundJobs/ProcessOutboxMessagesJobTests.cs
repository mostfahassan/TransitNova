...
{
    [Fact]
    public async Task ProcessOutboxMessagesJob_ValidMessage_Should_PublishAndMarkProcessed()
    {
    ...
    [Fact]
    public async Task ProcessOutboxMessagesJob_ProcessedMessage_Should_SkipPublishing()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync();
        var publisher = new Mock<IPublisher>();
        var message = CreateMessage(DateTime.UtcNow);
    ...
    [Fact]
    public async Task ProcessOutboxMessagesJob_TwentyFiveMessages_Should_ProcessOldestTwentyOnly()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync();
    ...
    [Fact]
    public async Task ProcessOutboxMessagesJob_UnknownEventType_Should_ThrowWithoutPublishing()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync();
        var publisher = new Mock<IPublisher>();
        var message = CreateMessage(DateTime.UtcNow);
        ...
        var act = () => CreateJob(fixture, publisher).Execute(CreateExecutionContext());

        await act.Should().ThrowAsync<InvalidOperationException>()
            .WithMessage("Unknown event type:*");
        publisher.Verify(x => x.Publish(It.IsAny<IDomainEvent>(), It.IsAny<CancellationToken>()), Times.Never);
        (await fixture.Context.OutboxMessages.AsNoTracking().SingleAsync()).ProcessedOn.Should().BeNull();
    }

    [Fact]
    public async Task ProcessOutboxMessagesJob_InvalidJson_Should_ThrowWithoutPublishing()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync();
        var publisher = new Mock<IPublisher>();
        var message = CreateMessage(DateTime.UtcNow);
    ...
    [Fact]
    public async Task ProcessOutboxMessagesJob_PublisherFailure_Should_PropagateAndLeaveMessageUnprocessedInDatabase()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync();
        var publisher = new Mock<IPublisher>();
        publisher.Setup(x => x.Publish(It.IsAny<IDomainEvent>(), It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task ProcessOutboxMessagesJob_SecondPublishFails_Should_NotPersistFirstProcessedMarker()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync();
        var publisher = new Mock<IPublisher>();
        ...
        await act.Should().ThrowAsync<InvalidOperationException>();
        var stored = await fixture.Context.OutboxMessages.AsNoTracking().ToListAsync();
        stored.Should().OnlyContain(x => x.ProcessedOn == null,
            "the batch SaveChanges is never reached after a publish failure");
    }

    [Fact]
    public async Task ProcessOutboxMessagesJob_CancelledToken_Should_ThrowWithoutPublishing()
    {
        await using var fixture = await SqliteAppDbContextFixture.CreateAsync();
        var publisher = new Mock<IPublisher>();
        fixture.Context.OutboxMessages.Add(CreateMessage(DateTime.UtcNow));
