...
namespace TransitNovaPayment.Busieness.Services.Payment.Handler.QueriesHandler
{
        ...
        : IQueryHandler<FilterPaymentsQuery, PagedResult<PaymentHistoryDetailsDto>>
    {
        ...
        {
            logger.LogInformation("Filtering payment histories. PageNumber: {PageNumber}, PageSize: {PageSize}.", request.Filter.PageNumber, request.Filter.PageSize);

            var paymentHistories = await paymentQueryRepository.FilterPaymentHistoryAsync(request.Filter, cancellationToken);

            logger.LogInformation("Payment histories filtered successfully. TotalCount: {TotalCount}.", paymentHistories.TotalCount);
