...
namespace TransitNova.InfraStructure.Repository.ShipmentRepo
{
    ...
    {
        public Task<bool> Editable(Guid shipmentId, ShipmentStatuses[] status, CancellationToken ct) 
                 => context.Shipments.Where(s => s.Id == shipmentId).AnyAsync(s => status.Contains(s.CurrentStatus), ct);

        public async Task<bool> Exists(Guid shipmentId , CancellationToken ct)
