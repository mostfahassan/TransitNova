...
namespace TransitNova.InfraStructure.Repository.CarrierRepo
{
    ...
    {
        public Task<bool> IsCarrierExists(Guid carrierId, CancellationToken ct = default)
         => context.Carriers.AnyAsync(c => c.Id == carrierId, ct);
        public async Task<bool> IsCarrierOwnerAsync(Guid carrierId, Guid userId, CancellationToken ct = default)
         => await context.Carriers.AnyAsync(c => c.Id == carrierId && c.AppUserId == userId, ct);
