...
namespace TransitNova.Domain.Entities.MainEntities
{
    public class Trip : AggregateRoot<Guid>
    {
        ...
        {
            var trip = new Trip(carrierId, warehouseId, tripType, shipments);
        ...
        public void Update(Guid carrierId, Guid warehouseId, TripType tripType,DateTime plannedDate, DateTime? startTime, DateTime? endTime, int totalShipments)
        {
            ...
            RaiseDomainEvent(new TripUpdatedDomainEvent(Id, TripType, Status, TotalShipments));
        }

        public void StartTrip(Guid operationManagerId, TripType type)
        {
            EnsurePlanned();
            EnsureType(type);
            ...
            RaiseDomainEvent(new TripStartedDomainEvent(Id, TripType, StartTime.Value));
        }
        ...
        public void AddShipment(Shipment shipment, Guid operationManagerId)
        {
            ...
            if (TripType == TripType.Pickup && shipment.CurrentStatus != ShipmentStatuses.AssignedToPickUpCarrier)
                throw new TripPlanningException("Pickup trips can only include shipments assigned to a pickup carrier.", Id);

            if (TripType == TripType.Delivery && shipment.CurrentStatus != ShipmentStatuses.InWarehouse)
        ...
        private static void ValidatePlan(Guid carrierId, Guid warehouseId, TripType tripType, List<Shipment> shipments)
        {
            ...
            if (tripType == TripType.Pickup && shipments.Any(s => s.CurrentStatus != ShipmentStatuses.AssignedToPickUpCarrier))
                throw new TripPlanningException("Pickup trips can only include shipments assigned to a pickup carrier.");

            if (tripType == TripType.Delivery && shipments.Any(s => s.CurrentStatus != ShipmentStatuses.InWarehouse))
