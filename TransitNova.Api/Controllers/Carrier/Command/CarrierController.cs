...
namespace TransitNova.Api.Controllers.Carrier.Command
{
    ...
    public class CarrierController(IMediator mediator ,IAuthorizationService authorizationService) : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateProfile([FromHeader(Name = "X-Idempotency-Key")] string requestId,  [FromBody] UpdateCarrierDto dto, CancellationToken ct)
        {
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [Consumes("application/json")]
        public async Task<IActionResult> AddCarrierInfo([FromHeader(Name = "X-Idempotency-Key")] string requestId, [FromBody] AdditionalInfoDto dto, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
