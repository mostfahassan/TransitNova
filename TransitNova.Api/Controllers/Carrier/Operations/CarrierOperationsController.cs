...
namespace TransitNova.Api.Controllers.Carrier.Operations
{
    ...
    public class CarrierOperationsController(IMediator mediator,IAuthorizationService authorizationService) : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateCarrierStatus([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid carrierId, [FromBody] ChangeCarrierStatus dto, CancellationToken ct)
        {
        ...
        public async Task<IActionResult> CompleteShipment([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid carrierId,Guid shipmentId, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
        ...
        public async Task<IActionResult> CompletePickupShipment([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid carrierId, Guid shipmentId, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
