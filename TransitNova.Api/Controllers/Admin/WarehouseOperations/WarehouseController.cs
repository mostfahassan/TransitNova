...
namespace TransitNova.Api.Controllers.Admin.WarehouseOperations
{
    ...
    public sealed class WarehouseController(IMediator mediator) : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateWarehouse([FromHeader(Name = "X-Idempotency-Key")] string requestId, [FromBody] CreateWarehouseDto dto, CancellationToken ct)
        {
        ...
        public async Task<IActionResult> UpdateWarehouse([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid warehouseId, [FromBody] UpdateWarehouseDto dto, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
        ...
        public async Task<IActionResult> DeleteWarehouse([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid warehouseId, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
        ...
        public async Task<IActionResult> Warehouse(Guid warehouseId, CancellationToken ct)
        {
