...
namespace TransitNova.Api.Controllers.Admin.Location
{
    ...
    public sealed class GovernmentsController(IMediator mediator) : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateGovernment([FromHeader(Name = "X-Idempotency-Key")] string requestId, [FromBody] CreateGovernmentDto dto, CancellationToken ct)
        {
        ...
        public async Task<IActionResult> UpdateGovernment([FromHeader(Name = "X-Idempotency-Key")] string requestId, int governmentId, [FromBody] UpdateGovernmentDto dto, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
        ...
        public async Task<IActionResult> DeleteGovernment([FromHeader(Name = "X-Idempotency-Key")] string requestId, int governmentId, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
        ...
        public async Task<IActionResult> Government(int governmentId, CancellationToken ct)
        {
        ...
        public async Task<IActionResult> Governments(CancellationToken ct)
        {
