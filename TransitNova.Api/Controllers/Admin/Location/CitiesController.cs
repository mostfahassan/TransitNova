...
namespace TransitNova.Api.Controllers.Admin.Location
{
    ...
    public sealed class CitiesController(IMediator mediator) : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateCity([FromHeader(Name = "X-Idempotency-Key")] string requestId, [FromBody] CreateCityDto dto, CancellationToken ct)
        {
        ...
        public async Task<IActionResult> UpdateCity([FromHeader(Name = "X-Idempotency-Key")] string requestId, int cityId, [FromBody] UpdateCityDto dto, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
        ...
        public async Task<IActionResult> DeleteCity([FromHeader(Name = "X-Idempotency-Key")] string requestId, int cityId, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
        ...
        public async Task<IActionResult> City(int cityId, CancellationToken ct)
        {
        ...
        public async Task<IActionResult> Cities([FromQuery] CityFilterDto filter, CancellationToken ct)
        {
