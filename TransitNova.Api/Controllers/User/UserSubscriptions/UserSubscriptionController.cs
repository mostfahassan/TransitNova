...
namespace TransitNova.Api.Controllers.User.User
{
    ...
    public class UserSubscriptionController(IMediator mediator) : ControllerBase
    {
        ...
        [HttpPost("bundles/{bundleId:int}/subscription")]
        [MapToApiVersion("1.0")]
        ...
        public async Task<IActionResult> SubscribeToBundle([FromHeader(Name = "X-Idempotency-Key")] string requestId, int bundleId, CancellationToken ct)
        {
        ...
        [EnableRateLimiting("CommandsLimiter")]
        [HttpDelete("bundles/{bundleId:int}/subscription")]
        [MapToApiVersion("1.0")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        ...
        public async Task<IActionResult> UnSubscribeFromBundle([FromHeader(Name = "X-Idempotency-Key")] string requestId, int bundleId, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
