...
namespace TransitNova.Api.Controllers.User.UserShipmentOperations
{
    ...
    {     
        ...
        public async Task<IActionResult> UpdateShipment([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid shipmentId, [FromBody] UpdateShipmentDto dto, CancellationToken ct)
        {
        ...
        public async Task<IActionResult> CancelShipment([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid shipmentId, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
        ...
        public async Task<IActionResult> IssueShipment([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid shipmentId, [FromBody] IssueShipmentReason issue, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
        ...
        public async Task<IActionResult> DeleteShipment([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid shipmentId, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();
