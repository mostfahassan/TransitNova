...
public sealed class CacheKeysTests
{
    ...
    {
        var key = CacheKeys.ShipmentByTrackingNumber("TRK 1/2");
    ...
    {
        var first = CacheKeys.FilterCarriers(new { Page = 1, Search = "Cairo" });
        var second = CacheKeys.FilterCarriers(new { Page = 1, Search = "Cairo" });
    ...
    public void OperationManagerHandledCarriers_Should_IncludeIdentityAndPagination_When_Called()
    {
        ...
        var key = CacheKeys.OperationManagerHandledCarriers(managerId, 2, 25);
