...
public sealed class SupportingEntityTests
{
    ...
    public void CreatePayment_Should_PreservePaymentDetails_When_DataIsValid()
    {
        ...
        var payment = Payment.Create(500m, 50m, PaymentMethod.Cash, PaymentSatus.Pending, "Collect on delivery", shipmentId);

        payment.Id.Should().NotBeEmpty();
        payment.Amount.Should().Be(500m);
        payment.Commission.Should().Be(50m);
        payment.PaymentMethod.Should().Be(PaymentMethod.Cash);
        payment.Status.Should().Be(PaymentSatus.Pending);
        payment.Notes.Should().Be("Collect on delivery");
        payment.ShipmentId.Should().Be(shipmentId);
        payment.CurrentState.Should().BeTrue();
    }
