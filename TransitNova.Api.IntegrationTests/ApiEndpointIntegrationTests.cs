...
public sealed class ApiEndpointIntegrationTests : IClassFixture<TransitNovaWebApplicationFactory>, IAsyncLifetime
{
    ...
    private readonly TransitNovaWebApplicationFactory factory;
    ...
    {
        var endpoints = ControllerEndpointCatalog.Discover(factory.Services);

        endpoints.Should().HaveCount(101);
        endpoints
    ...
    [Fact]
    public async Task EveryControllerEndpoint_Should_ExecuteThroughTheHttpPipeline()
    {
        var endpoints = ControllerEndpointCatalog.Discover(factory.Services);
    ...
    [Fact]
    public async Task ProtectedControllerEndpoints_Should_ReturnUnauthorized_When_RequestIsAnonymous()
    {
        var endpoints = ControllerEndpointCatalog.Discover(factory.Services)
    ...
    [Fact]
    public async Task PublicControllerEndpoints_Should_Not_RequireAuthentication()
    {
        var endpoints = ControllerEndpointCatalog.Discover(factory.Services)
            .Where(endpoint => !endpoint.RequiresAuthorization)
            .ToArray();
    ...
    [Fact]
    public async Task StateChangingEndpoints_Should_ReturnBadRequest_When_IdempotencyKeyIsInvalid()
    {
        var endpoints = ControllerEndpointCatalog.Discover(factory.Services)
            .Where(endpoint => endpoint.HttpMethod is "POST" or "PUT" or "PATCH" or "DELETE")
            .ToArray();
    ...
    [Fact]
    public async Task HealthEndpoint_Should_ReturnSuccess_When_TestDatabaseIsAvailable()
    {
        using var client = factory.CreateAnonymousClient();
