...
{
    [Fact]
    public async Task GetShipmentByIdHandler_WhenShipmentExists_ShouldReturnRepositoryDto()
    {
    ...
    [Fact]
    public async Task GetShipmentByIdHandler_WhenShipmentDoesNotExist_ShouldReturnNotFound()
    {
        var repository = new Mock<IShipmentQueryRepository>();
    ...
    [Fact]
    public async Task GetShipmentByIdHandler_WhenCancellationTokenIsProvided_ShouldForwardTokenToRepository()
    {
        var repository = new Mock<IShipmentQueryRepository>();
    ...
    [Fact]
    public async Task GetShipmentStatisticsHandler_WhenStatisticsExist_ShouldReturnSameStatusCounts()
    {
        var repository = new Mock<IShipmentQueryRepository>();
        ...
        repository.Setup(x => x.GetShipmentCountInStatus(It.IsAny<CancellationToken>())).ReturnsAsync(statistics);
    ...
    [Fact]
    public async Task GetShipmentStatisticsHandler_WhenNoStatisticsExist_ShouldReturnSuccessWithEmptyDictionary()
    {
        var repository = new Mock<IShipmentQueryRepository>();
        repository.Setup(x => x.GetShipmentCountInStatus(It.IsAny<CancellationToken>())).ReturnsAsync([]);
        var handler = new GetShipmentStatisticsHandler(Mock.Of<ILogger<GetShipmentStatisticsHandler>>(), repository.Object);
    ...
    public async Task TrackShipmentQueryHandler_WhenRepositoryResultIsKnown_ShouldReturnExpectedStatus(bool exists, ResultStatus expectedStatus)
    {
        var repository = new Mock<IShipmentQueryRepository>();
    ...
    [InlineData(true, ResultStatus.Success)]
    [InlineData(false, ResultStatus.NotFound)]
    public async Task GetUserShipmentHandler_WhenRepositoryResultIsKnown_ShouldReturnExpectedStatus(bool exists, ResultStatus expectedStatus)
    {
