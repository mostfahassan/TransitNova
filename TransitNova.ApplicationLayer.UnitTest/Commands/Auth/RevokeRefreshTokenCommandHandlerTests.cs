...
public sealed class RevokeRefreshTokenCommandHandlerTests
{
    ...
    [InlineData(1)]
    [InlineData(5)]
    public async Task RevokeRefreshTokenHandler_WhenRepositoryCompletes_ShouldReturnSuccessRegardlessOfAffectedRows(int affectedRows)
    {
    ...
    [Fact]
    public async Task RevokeRefreshTokenHandler_WhenCalled_ShouldRevokeTokensForCommandUser()
    {
        var repository = new Mock<IRefreshTokenRepository>();
    ...
    [Fact]
    public async Task RevokeRefreshTokenHandler_WhenCancellationTokenIsPassed_ShouldForwardIt()
    {
        var repository = new Mock<IRefreshTokenRepository>();
        var handler = new RevokeRefreshTokenHandler(repository.Object);
    ...
    [Fact]
    public async Task RevokeRefreshTokenHandler_WhenRepositoryFails_ShouldPropagateException()
    {
        var repository = new Mock<IRefreshTokenRepository>();
        repository.Setup(x => x.RevokeAllUserTokenAsync(
                It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
