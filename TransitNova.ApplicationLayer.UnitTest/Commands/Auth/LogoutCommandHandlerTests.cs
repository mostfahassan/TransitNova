...
{
    [Fact]
    public async Task SignOutHandler_WhenCalled_ShouldReturnSuccess()
    {
    ...
    [Fact]
    public async Task SignOutHandler_WhenCalled_ShouldSignOutExactlyOnce()
    {
        var rules = new Mock<IUserAuthRulesService>();
        var handler = CreateHandler(rules);

        await handler.Handle(new SignOutCommand(Guid.NewGuid()), CancellationToken.None);

        rules.Verify(x => x.SignOutAsync(CancellationToken.None), Times.Once);
    }

    [Fact]
    public async Task SignOutHandler_WhenCancellationTokenIsPassed_ShouldForwardIt()
    {
        var rules = new Mock<IUserAuthRulesService>();
        var handler = CreateHandler(rules);
        using var cancellation = new CancellationTokenSource();

        await handler.Handle(new SignOutCommand(Guid.NewGuid()), cancellation.Token);

        rules.Verify(x => x.SignOutAsync(cancellation.Token), Times.Once);
    }

    [Fact]
    public async Task SignOutHandler_WhenIdentitySignOutFails_ShouldPropagateException()
    {
        var rules = new Mock<IUserAuthRulesService>();
