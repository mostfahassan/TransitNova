...
{
    [Fact]
    public async Task RegistrationCommandValidator_WhenAllFieldsAreValid_ShouldPassValidation()
    {
    ...
    [InlineData("1invalid")]
    public async Task RegistrationCommandValidator_WhenUsernameViolatesPolicy_ShouldFailValidation(string username)
    {
        var dto = ValidDto();
    ...
    [InlineData("missing@")]
    public async Task RegistrationCommandValidator_WhenEmailIsInvalid_ShouldFailValidation(string email)
    {
        var dto = ValidDto();
        dto.Email = email;
    ...
    public async Task RegistrationCommandValidator_WhenPasswordViolatesSecurityPolicy_ShouldFailValidation(string password)
    {
        var dto = ValidDto();
    ...
    public async Task RegistrationCommandValidator_WhenPhoneIsInvalid_ShouldFailValidation(string phone)
    {
        var dto = ValidDto();
    ...
    public async Task RegistrationCommandValidator_WhenRequiredProfileValueIsInvalid_ShouldFailValidation(string field)
    {
        var dto = ValidDto();
        switch (field)
        {
    ...
    private static RegistrationCommand Command(RegisterDto dto) => new(Guid.NewGuid(), dto);
