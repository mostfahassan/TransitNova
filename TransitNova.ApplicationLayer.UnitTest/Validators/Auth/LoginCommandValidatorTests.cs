...
public sealed class LoginCommandValidatorTests
{
    ...
    public async Task LoginCommandValidator_WhenCredentialsAreProvided_ShouldReturnExpectedValidity(
        string email,
        string password,
        bool expectedValidity)
    {
        var validator = new LoginCommandValidator(new LoginValidator());
        var command = new LoginCommand(Guid.NewGuid(), new LoginDto(password, email));
