...
namespace TransitNovaUI.BusinessLayer.ApiImplementation.OperationManager.Shipments.Query
{
        ...
        : ApiServiceBase(httpHandler, httpClient), IOperationManagerShipmentsQuery
    {
        ...
        {
            var url = httpHandler.UrlBuilder(ApiRoutes.Build(ApiRoutes.OperationManagerShipments.FilterShipmentsUrl,
        ...
        {
            var url = httpHandler.UrlBuilder(ApiRoutes.Build(ApiRoutes.OperationManagerShipments.GetAssignedShipmentsUrl,
                ("Status", filter.Status),
                ("Mode", filter.Mode),
                ("From", filter.From),
                ("To", filter.To),
        ...
        public Task<ApiResponse<IEnumerable<UiRetrieveShipmentStatusDto>>> GetShipmentHistoriesAsync(Guid shipmentId, string bearerToken, CancellationToken cancellationToken = default)
        {
            ...
            return SendQueryRequestAsync<IEnumerable<UiRetrieveShipmentStatusDto>>(HttpMethod.Get, url, bearerToken, cancellationToken, null, null);
        }
        ...
        {
            var url = httpHandler.UrlBuilder(ApiRoutes.Build(ApiRoutes.OperationManagerShipments.GetShipmentReviewQueueUrl,
                ("Status", filter.Status),
                ("Mode", filter.Mode),
                ("From", filter.From),
                ("To", filter.To),
