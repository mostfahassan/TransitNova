...
namespace TransitNova.BusinessLayer.Features.UserOperations.Handlers.CommandsHandler
{
    public sealed class UnSubscribeBundleHandler(
        ...
        ILogger<UnSubscribeBundleHandler> logger)
        : ICommandHandler<UnSubscribeBundleCommand, BaseResult>
    {
        public async Task<BaseResult> Handle(UnSubscribeBundleCommand request, CancellationToken cancellationToken)
        {
            //====== Verify User Exists ======
            var userId = await userRepository.GetAppUserId(request.UserId, cancellationToken);
