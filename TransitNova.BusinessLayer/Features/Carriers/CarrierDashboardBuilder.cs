...
namespace TransitNova.BusinessLayer.Features.Carriers
{
    ...
    {
        public static CarrierDashboardDto Build
        (Dictionary<ShipmentStatuses, int> stats, List<CarrierTripDto> trips, decimal totalRevenue, Carrier carrier, IEnumerable<RetrieveShipmentDto> recentShipments)
        {
            ...
            return new CarrierDashboardDto
            {
                ...
                RecentShipments = recentShipments.ToList(),
                ActiveTrips = activeTrips,
                ShipmentStatistics = stats.Select(s => new CarrierStatusStatDto { Status = s.Key, Count = s.Value }).ToList(),
