...
namespace TransitNova.BusinessLayer.Services.ShipmentServices
{
    ...
    {
        public async Task<Guid> PrepareShipmentCreation(CreateShipmentDto Dto, Guid AppUserId, CancellationToken cancellationToken)
        {
            var senderId = await user.GetAppUserId(AppUserId, cancellationToken);
            ...
            //==== Create Receiver =========//
            await receiver.CreateReceiver(receiverToCreate, cancellationToken);
