...
namespace TransitNova.BusinessLayer.Interfaces.Repositories.OperationManagerRepository
{
    public interface IOperationManagerQueryRepository
    {
        ...
        Task<PagedResult<CarrierSummaryDetailsDto>> RetrieveHandledCarriersByOperationManager(Guid operationManagerId, int pageNumber, int pageSize, CancellationToken cancellationToken);
        Task<PagedResult<RetrieveShipmentSummaryDto>> RetrieveHandledShipmentsByOperationManager(Guid operationManagerId, int pageNumber, int pageSize, CancellationToken cancellationToken);
    }
