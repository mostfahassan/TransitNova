...
namespace TransitNova.BusinessLayer.Features.Reports.Handler
{
    public sealed class GenerateInvoiceReportHandler(
        ...
        ILogger<GenerateInvoiceReportHandler> logger)
        : ICommandHandler<GenerateInvoiceReportCommand, BaseResult>
    {
        public async Task<BaseResult> Handle(GenerateInvoiceReportCommand request, CancellationToken cancellationToken)
        {
            var payloadJson = ReportPayloadSerializer.Serialize(request.Contract);
            var reportRequest = ReportRequest.CreateReport(InvoiceReportContract.ReportKey, payloadJson, request.RequestedBy);
            ...
            return BaseResult.Created("Invoice report request created successfully.");
        }
    }
}
