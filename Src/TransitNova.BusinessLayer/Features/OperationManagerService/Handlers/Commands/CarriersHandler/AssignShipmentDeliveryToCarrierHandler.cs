...
namespace TransitNova.BusinessLayer.Features.OperationManagerService.Handlers.Commands.CarriersHandler
{
    public class AssignShipmentDeliveryToCarrierHandler(
        ...
        : ICommandHandler<AssignShipmentDeliveryToCarrierCommand, BaseResult>
    {
        public async Task<BaseResult> Handle(AssignShipmentDeliveryToCarrierCommand request, CancellationToken cancellationToken)
        {
           ...
           var assignedShipmentTrackingNumber = await shipmentAssignment.AssignDeliveryAsync(request.ShipmentId,request.OperationManagerId, request.CarrierId, cancellationToken);
            ...
            logger.LogInformation("Shipment with ID {ShipmentId} assigned to delivery carrier with ID {CarrierId} by operation manager with ID {OperationManagerId}.", request.ShipmentId, request.CarrierId, request.OperationManagerId);
