...
namespace TransitNova.BusinessLayer.Interfaces.Repositories.ShipmentRepository
{
    public interface IShipmentQueryRepository
    {
        ...
        Task<RetrieveShipmentDto?> GetShipmentAsync(Expression<Func<Shipment, bool>> predicate, CancellationToken ct);
        ...
        Task<Shipment?> GetShipmentForCommandsAsync(Guid shipmentId, CancellationToken ct);
        Task<string?> GetShipmentTrackingNumberAsync(Guid shipmentId, CancellationToken ct);
