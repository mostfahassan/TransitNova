        ...
        : INotificationHandler<ShipmentCreatedDomainEvent>
    {
        public async Task Handle(ShipmentCreatedDomainEvent notification, CancellationToken cancellationToken)
        {
            var operationManagersIds =
                await operationManager.GetOperationManagersAppUserIdsAsync(cancellationToken);
            ...
            {
                var senderNotification = Notification.Create(shipment.Sender.AppUserId, "Shipment Created Successfully", $"Your shipment {notification.TrackingNumber} has been created successfully.");
