...
namespace TransitNova.Api.Controllers.User.UserSubscriptions
{
    ...
    public class UserSubscriptionController(IMediator mediator) : ControllerBase
    {
        ...
        public async Task<IActionResult> SubscribeToBundleAsync([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid bundleId, CancellationToken ct)
        {
        ...
        public async Task<IActionResult> UnsubscribeFromBundleAsync([FromHeader(Name = "X-Idempotency-Key")] string requestId, Guid bundleId, CancellationToken ct)
        {
            if (!Guid.TryParse(requestId, out Guid parsedRequestId))
                return BadRequest();

            var userId = User.GetUserId();
            var response = await mediator.Send(new UnsubscribeFromBundleCommand(parsedRequestId, userId, bundleId), ct);
            return response.ToActionResult();
        }
