...
namespace TransitNova.Api.Controllers.Admin.Subscriptions
{
    ...
    public sealed class SubscriptionsController(IMediator mediator) : ControllerBase
    {
        ...
        [EndpointName("Get Subscribers")]
        [EndpointSummary("Get all bundle subscribers")]
        [EndpointDescription("Returns all bundle subscriptions for the admin subscriptions index.")]
        public async Task<IActionResult> SubscribersAsync(CancellationToken ct)
        {
            var response = await mediator.Send(new GetSubscribersQuery(), ct);
        ...
        [EndpointDescription("Returns all bundle subscriptions for a specific bundle.")]
        public async Task<IActionResult> BundleSubscribersAsync(Guid bundleId, CancellationToken ct)
        {
