...
namespace TransitNova.InfraStructure.Common.RolesService
{
    ...
    {
        public async Task AddNewRoleAsync(string roleName, CancellationToken cancellationToken = default)
        {
        ...
        public async Task UpdateRoleAsync(Guid roleId, string newRoleName, CancellationToken cancellationToken = default)
        {
            var role = await roleManager.FindByIdAsync(roleId.ToString());
