...
{
    internal sealed class BundleInvoicePdfDocument(BundlePaymentInvoiceDto invoice) : IDocument
    {
        public void Compose(IDocumentContainer container)
        {
            container.Page(page =>
            {
                ...
                page.Header().ComposeCompanyHeader(
                    "Bundle Invoice Report",
                    "Subscription billing summary and bundle entitlement record",
                    invoice.FullName,
                ...
                page.Content().PaddingVertical(18).Column(column =>
                {
                    ...
                    column.Item().Element(ComposeSubscriberAndBundle);
        ...
        private void ComposeHero(IContainer container)
        {
            container.Row(row =>
            {
                ...
                row.RelativeItem().Element(PdfDocumentDesign.Card).Column(column =>
                {
                    column.Item().Text("Subscription Status").FontSize(8).SemiBold().FontColor(PdfPalette.Muted);
                    column.Item().PaddingTop(4).Text(invoice.Status).FontSize(22).Bold()
        ...
        private void ComposeSubscriberAndBundle(IContainer container)
        {
            container.Row(row =>
            {
                row.RelativeItem().Element(PdfDocumentDesign.Card).Column(column =>
                {
                    column.Item().Text("Subscriber Information").FontSize(12).Bold().FontColor(PdfPalette.Navy);
                    column.Item().PaddingTop(10).ComposeInfoLine("Subscriber", PdfDocumentDesign.ValueOrDash(invoice.FullName));
                    column.Item().PaddingTop(5).ComposeInfoLine("Payment Method", invoice.PaymentMethod);
                    column.Item().PaddingTop(5).ComposeInfoLine("Reference Type", invoice.ReferenceType);
                    column.Item().PaddingTop(5).ComposeInfoLine("Reference ID", invoice.ReferenceId.ToString());
                });
                ...
                row.RelativeItem().Element(PdfDocumentDesign.Card).Column(column =>
                {
                    column.Item().Text("Bundle Information").FontSize(12).Bold().FontColor(PdfPalette.Navy);
                    column.Item().PaddingTop(10).ComposeInfoLine("Bundle", invoice.BundleName);
                    column.Item().PaddingTop(5).ComposeInfoLine("Bundle ID", invoice.BundleId.ToString());
                    column.Item().PaddingTop(5).ComposeInfoLine("Subscribed At", PdfDocumentDesign.Date(invoice.SubscribedAt));
                    column.Item().PaddingTop(5).ComposeInfoLine("End Date", PdfDocumentDesign.Date(invoice.EndDate));
                });
            });
        }
        ...
        private void ComposeInvoiceItems(IContainer container)
        {
            container.Element(PdfDocumentDesign.Card).Column(column =>
            {
                ...
                column.Item().PaddingTop(10).Table(table =>
                {
                    ...
                    AddInvoiceRow(table, 0, $"{invoice.BundleName} subscription", 1, invoice.BundlePrice);
        ...
        private void ComposeTotalsAndVerification(IContainer container)
        {
            container.Row(row =>
            {
                row.RelativeItem().Element(PdfDocumentDesign.Card).Column(column =>
                {
                    ...
                    {
                        inner.RelativeItem().Height(86).ComposePlaceholder("QR Code", "Subscription verification placeholder");
                        inner.ConstantItem(10);
                        inner.RelativeItem().Height(86).ComposePlaceholder("Barcode", invoice.InvoiceId ?? PdfDocumentDesign.ShortId(invoice.PaymentId));
                    });
                });
                ...
                row.RelativeItem().Element(PdfDocumentDesign.Card).Column(column =>
                {
                    ...
                    column.Item().PaddingTop(10).ComposeInfoLine("Subtotal", PdfDocumentDesign.Money(invoice.BundlePrice + invoice.Commission, invoice.Currency));
